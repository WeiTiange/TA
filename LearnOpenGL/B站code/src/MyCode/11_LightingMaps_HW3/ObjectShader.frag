...
void main() {
    ...
    vec3 var_emis = texture(material.emission, vec2(UV.x, UV.y + time)).rgb;
    // 环境光
    ...
    // 自发光
    vec3 emission = var_emis * material.emissionInt * (sin(time) + 1) / 2;

    // 最终输出
    ambient *= light.ambientInt;
    diffuse *= light.diffuseInt;
    specular *= light.specularInt;
    vec3 finalRGB = ambient + diffuse + specular + emission;
