// Texture练习2
// 尝试用不同的纹理环绕方式，设定一个从0.0f到2.0f范围内的（而不是原来的0.0f到1.0f）纹理坐标。试试看能不能在箱子的角落放置4个坨梓
...
//  ---- Texture2----
    ...
    // 创建和编译自定义的shader程序
    Shader customShader("./src/MyCode/04_Texture_HW2/3.3.shader.vert", "./src/MyCode/04_Texture_HW2/3.3.shader.frag");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        customShader.use();
        customShader.setFloat("time", (sin((glfwGetTime() + 2 * M_PI)/4)+1.0f) / 2.0);
