...
vec3 CalcSpotLight(SpotLight light, vec3 nDirWS, vec3 posWS, vec3 vDirWS) {
    ...
    // 漫反射
    float diff = max(dot(nDirWS, lDirWS), 0.0);
    vec3 diffuse = diff * T_diffuse * light.diffuse * attenuation * intensity; 

    // 镜面反射
    float spec = pow(max(dot(lrDirWS, vDirWS), 0.0), material.shininess);
    vec3 specular = spec * T_specular * light.specular * attenuation * intensity;

    // 环境光照
    vec3 ambient = T_diffuse * light.ambient * attenuation * intensity;
