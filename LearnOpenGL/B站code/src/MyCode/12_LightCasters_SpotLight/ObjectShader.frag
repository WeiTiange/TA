...
void main() {
    float theta = dot(-light.direction, normalize(light.position - posWS));
    float epsilon = light.innerCutOffAngle - light.outerCutOffAngle;
    float spotlightInt = clamp((theta - light.outerCutOffAngle)/epsilon, 0.0, 1.0);
    // 贴图载入
    ...
    vec3 finalRGB = ambient + (diffuse + specular) * spotlightInt * LightAttenuation(posWS);
