...
// 因为左下角传递的顶点位置值是 (-0.5, -0.5, 0.0),由于颜色值会被钳制到0-1的范围内(LDR),-0.5就变成了0,反映到颜色上就是黑的。
...
int main()
{
    ...
    // 创建和编译自定义的shader程序
    Shader customShader("./src/MyCode/03_Shader_HW3/3.3.shader.vert", "./src/MyCode/03_Shader_HW3/3.3.shader.frag");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        customShader.use();
        customShader.setFloat("horizontalOffset", float(sin(glfwGetTime())/2.0f));
        customShader.setFloat("vertVerticalOffset", float(sin(glfwGetTime())));
        glBindVertexArray(VAO);
