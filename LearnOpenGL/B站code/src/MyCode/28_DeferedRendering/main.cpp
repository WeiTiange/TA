...
int main() {
    ...
    Shader LightingShader("./src/MyCode/28_DeferedRendering/Shaders/Lighting.vert", "./src/MyCode/28_DeferedRendering/Shaders/Lighting.frag");
    Shader LightSourceShader("./src/MyCode/28_DeferedRendering/Shaders/LightSource.vert", "./src/MyCode/28_DeferedRendering/Shaders/LightSource.frag");
    ...
    objectPositions.push_back(glm::vec3(-4.0, -2.0, -4.0));
    objectPositions.push_back(glm::vec3(0.0, -2.0, -4.0));
    objectPositions.push_back(glm::vec3(4.0, -2.0, -4.0));
    objectPositions.push_back(glm::vec3(-4.0, -2.0, 0.0));
    ...
    for (GLuint i = 0; i < NR_LIGHTS; i++)
    {
        ...
        GLfloat yPos = ((rand() % 100) / 100.0) * 4.0 - 4.0;
    ...
    FBOTextureAttach(gPosition, GL_RGB16F, GL_RGB, 0);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        for (int i = 0; i < lightPositions.size(); i++) {
            LightingShader.setVec3("lights[" + to_string(i) + "].Position", lightPositions[i]);
            LightingShader.setVec3("lights[" + to_string(i) + "].Color", lightColors[i] * 10.0f);
        }
        renderQuad();
        ...
        LightSourceShader.setMat4("V", V);
        LightSourceShader.setMat4("P", P);
        for (int i = 0; i < lightPositions.size(); i++) {
            glm::mat4 M = glm::mat4(1.0f);
            M = glm::translate(M, lightPositions[i]);
            M = glm::scale(M, glm::vec3(0.3f));
            LightSourceShader.setMat4("M", M);
